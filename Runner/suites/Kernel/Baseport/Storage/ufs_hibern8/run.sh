...
if LINK_STATE_NODE=$(ufs_get_dt_node_path "/sys/bus/platform/devices/$PRIMARY_UFS_CONTROLLER/power_info/link_state"); then
    log_info "Found UFS link state node: $LINK_STATE_NODE"
else
    log_skip "Failed to get UFS link state node"
    echo "$TESTNAME SKIP" > "$res_file"
    exit 0
fi
...
# Use ufs_check_node_status to verify ufs link state
if ! ufs_check_node_status "$LINK_STATE_NODE" "ACTIVE" 10 1; then
...
# Wait for UFS to go to Hibern8 state
LINK_STATUS_CHECK_NO_LOAD=0
# Use ufs_check_node_status_with_timeout to verify link state is HIBERN8
if ! ufs_check_node_status_with_timeout "$LINK_STATE_NODE" "HIBERN8" 180; then
