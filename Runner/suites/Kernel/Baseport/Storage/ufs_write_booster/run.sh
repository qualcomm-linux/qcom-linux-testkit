...
# Read Write Booster status during load
WB_STATUS=$(cat "$WB_STATUS_NODE" 2>/dev/null | tr -d '[:space:]')
log_info "Write Booster status under load: $WB_STATUS"

# Verify the Write Booster is enabled (value should be 1)
if [ "$WB_STATUS" != "1" ]; then
    log_fail "UFS Write Booster is not enabled during load. Current status: $WB_STATUS (Expected: 1)"
...
log_pass "UFS Write Booster is enabled during load"
...
log_info "Write Booster status during load: $WB_STATUS (Expected: 1)"
