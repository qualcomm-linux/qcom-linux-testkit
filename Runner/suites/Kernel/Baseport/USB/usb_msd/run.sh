...
if [ "$msd_device_count" -gt 0 ]; then
    if [ "${has_devnodes_count:-0}" -gt 0 ] 2>/dev/null; then
        log_pass "$TESTNAME : Test Passed - USB Mass Storage device(s) detected with block device associated"
        echo "$TESTNAME PASS" > "$RES_FILE"
        exit 0
    else
        log_fail "$TESTNAME : Test Failed - MSD interface(s) detected but no associated block device found"
