...
if ! check_dependencies "find grep basename awk cut"; then
...
if [ ! -f "$BINARY_PATH" ]; then
    ...
    log_info "  ./run.sh --binary-path /path/to/dmabuf-heap"
    pass=false
    echo "$TESTNAME SKIP" > "$res_file"
    exit 0
fi
...
output_file="./dmabuf_heap_kselftest.log"
...
if ! grep -q "Testing allocation and importing"; then
    echo "$TESTNAME SKIP" > "$res_file"
    exit 0
fi
...
if grep -q "# Totals:" "$output_file"; then
    ...
    skip_count=$(grep -c "^ok .*# SKIP" "$output_file" || echo 0)
