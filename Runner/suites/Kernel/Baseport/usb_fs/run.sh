...
# Validate that at least one non-hub USB peripheral is enumerated at Full Speed (12 Mb/s).
...
log_info "=== Detecting non-hub Full Speed USB devices ==="
...
#  - Consider a device Full Speed if speed (Mb/s) equals 12.
...
for d in /sys/bus/usb/devices/*; do
    ...
    if [ "$speed_int" -eq 12 ]; then
        fs_count=$((fs_count + 1))
    fi
done

echo "Full Speed (12 Mb/s) device count: $fs_count"
...
if [ "$fs_count" -gt 0 ]; then
    log_pass "$TESTNAME : Test Passed - $fs_count Full Speed device(s) found among $non_hub_count non-hub device(s)."
    echo "$TESTNAME PASS" > "$res_file"
    exit 0
else
    log_fail "$TESTNAME : Test Failed - No non-hub USB device enumerated at Full Speed (12 Mb/s)."
    echo "$TESTNAME FAIL" > "$res_file"
    exit 1
fi
