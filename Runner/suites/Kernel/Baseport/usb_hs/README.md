...
This shell script executes on the DUT (Device-Under-Test) and verifies successful enumeration of connected USB Devices in High Speed (HS - 480 Mb/s).

---

## Setup

- Connect HS USB peripheral(s) to USB port(s) on DUT.
...
cd <path-on-device>/Runner && ./run-test.sh usb_hs
```
