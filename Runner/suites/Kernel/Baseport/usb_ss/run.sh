...
# Validate that at least one non-hub USB peripheral is enumerated at SuperSpeed (>= 5000 Mb/s).
...
log_info "=== Detecting non-hub Superspeed USB devices ==="
...
#  - Consider a device SuperSpeed if speed (Mb/s) is >= 5000.
...
for d in /sys/bus/usb/devices/*; do
    ...
    if [ "$speed_int" -ge 5000 ]; then
        ss_count=$((ss_count + 1))
    fi
done

echo "SuperSpeed (>=5000 Mb/s) device count: $ss_count"
...
if [ "$ss_count" -gt 0 ]; then
    log_pass "$TESTNAME : Test Passed - $ss_count SuperSpeed device(s) found among $non_hub_count non-hub device(s)."
    echo "$TESTNAME PASS" > "$res_file"
    exit 0
else
    log_fail "$TESTNAME : Test Failed - No non-hub USB device enumerated at SuperSpeed (>= 5000 Mb/s)."
    echo "$TESTNAME FAIL" > "$res_file"
    exit 1
fi
