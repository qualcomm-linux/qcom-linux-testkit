...
# Validate that at least one non-hub USB peripheral is enumerated at SuperSpeedPlus (>= 10000 Mb/s).
...
log_info "=== Detecting non-hub Superspeed Plus USB devices ==="
...
#  - Consider a device SuperSpeedPlus if speed (Mb/s) is >= 10000.
...
for d in /sys/bus/usb/devices/*; do
    ...
    if [ "$speed_int" -ge 10000 ]; then
        ssp_count=$((ssp_count + 1))
    fi
done

echo "SuperSpeedPlus (>=10000 Mb/s) device count: $ssp_count"
...
if [ "$ssp_count" -gt 0 ]; then
    log_pass "$TESTNAME : Test Passed - $ssp_count SuperSpeedPlus device(s) found among $non_hub_count non-hub device(s)."
    echo "$TESTNAME PASS" > "$res_file"
    exit 0
else
    log_fail "$TESTNAME : Test Failed - No non-hub USB device enumerated at SuperSpeedPlus (>= 10000 Mb/s)."
    echo "$TESTNAME FAIL" > "$res_file"
    exit 1
fi
