...
DURATIONS	        Recording durations: short, medium, long (deprecated)	   short
...
--durations	                Recording durations: short, medium, long (deprecated, use --config-name instead)
...
**Example 6: Testing all 10 configs with short duration**
```
...
- The `--durations` option is deprecated; use `--config-name` or `--config-filter` for better control.
