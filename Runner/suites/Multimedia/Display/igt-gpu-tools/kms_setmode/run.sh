    ...
    case "$1" in
        --kms-setmode-path)
            ...
            if [ -n "${1:-}" ]; then
                ...
                log_error "Missing value for --kms-setmode-path parameter"
                echo "$TESTNAME FAIL" >"$result_file"
                exit 1
            fi
            ;;
        --help|-h)
            log_info "Usage: $0 [--kms-setmode-path BINARY_PATH] | [BINARY_PATH]"
...
if [ ! -x "$KMS_SETMODE_CMD" ]; then
    log_error "FAIL: kms_setmode binary not found or not executable at: $KMS_SETMODE_CMD"
    log_error "Please ensure IGT GPU tools are installed or specify the binary path with --kms-setmode-path"
    echo "$TESTNAME FAIL" > "$result_file"
    exit 1
fi

log_info "Using kms_setmode binary at: $KMS_SETMODE_CMD"
