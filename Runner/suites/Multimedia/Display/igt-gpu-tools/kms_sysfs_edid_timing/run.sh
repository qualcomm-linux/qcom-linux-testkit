...
while [ $# -gt 0 ]; do
    case "$1" in
        --kms-sysfs-edid-timing-path)
            shift
            if [ -n "${1:-}" ]; then
                KMS_SYSFS_EDID_TIMING_CMD="$1"
            else
                log_error "Missing value for --kms-sysfs-edid-timing-path parameter"
                echo "$TESTNAME FAIL" >"$result_file"
                exit 1
            fi
            ;;
        --help|-h)
            log_info "Usage: $0 [--kms-sysfs-edid-timing-path BINARY_PATH] | [BINARY_PATH]"
...
if [ ! -x "$KMS_SYSFS_EDID_TIMING_CMD" ]; then
    log_error "FAIL: kms_sysfs_edid_timing binary not found or not executable at: $KMS_SYSFS_EDID_TIMING_CMD"
    log_error "Please ensure IGT GPU tools are installed or specify the binary path with --kms-sysfs-edid-timing-path"
    echo "$TESTNAME FAIL" > "$result_file"
    exit 1
fi

log_info "Using kms_sysfs_edid_timing binary at: $KMS_SYSFS_EDID_TIMING_CMD"
...
"$KMS_SYSFS_EDID_TIMING_CMD" > "$log_file" 2>&1
RC="$?"
