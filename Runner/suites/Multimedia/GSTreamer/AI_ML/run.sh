...
TIMEOUT=60 # seconds
ENCODED_VIDEO_PATH="/opt/video_out.mp4"
GST_DEBUG_LEVEL=2

# ----------------------------------------------------------------------
#  GStreamer pipelines (kept on a single line for readability)
# ----------------------------------------------------------------------
DETECTION_PIPELINE='gst-launch-1.0 -e filesrc location=/opt/video.mp4 ! \
    qtdemux ! queue ! h264parse ! v4l2h264dec capture-io-mode=4 output-io-mode=4 \
    ! queue ! tee name=split \
    split. ! queue ! qtivcomposer name=mixer ! queue ! waylandsink sync=true fullscreen=true \
    split. ! queue ! qtimlvconverter ! queue ! qtimltflite delegate=external \
    external-delegate-path=libQnnTFLiteDelegate.so \
    external-delegate-options="QNNExternalDelegate,backend_type=htp;" \
    model=/opt/yolox_quantized.tflite ! queue ! qtimlpostprocess \
    module=yolov8 labels=/opt/labels/yolox.json settings="{\"confidence\": 50.0}" \
    ! video/x-raw,format=BGRA,width=640,height=360 ! queue ! mixer.'

CLASSIFICATION_PIPELINE='gst-launch-1.0 -e filesrc location=/opt/video.mp4 ! \
    qtdemux ! queue ! h264parse ! v4l2h264dec capture-io-mode=4 output-io-mode=4 \
    ! video/x-raw,format=NV12_Q08C ! queue ! tee name=split \
    ! queue ! qtimetamux name=metamux ! queue ! qtivoverlay ! queue \
    ! v4l2h264enc capture-io-mode=4 output-io-mode=5 ! h264parse ! queue \
    ! mp4mux ! queue ! filesink location='"${ENCODED_VIDEO_PATH}"' \
    split. ! queue ! qtimlvconverter ! queue ! qtimltflite delegate=external \
    external-delegate-path=libQnnTFLiteDelegate.so \
    external-delegate-options="QNNExternalDelegate,backend_type=htp;" \
    model=/opt/inception_v3_quantized.tflite ! queue ! qtimlpostprocess \
    results=5 module=mobilenet-softmax labels=/opt/labels/imagenet_labels.txt \
    settings="{\"confidence\": 31.0}" ! text/x-raw ! queue ! metamux.'
...
download_model_artifacts() {
    ...
    zip_path=$(download_resource \
        ...
        "${out}")   # dest is a directory → original name kept (labels.zip)
    extract_zip_to_dir "${zip_path}" "${out}"
...
# ----------------------------------------------------------------------
#  Wayland environment handling (if available)
# ----------------------------------------------------------------------
log_info "Setting up Wayland environment (if available)"
if command -v discover_wayland_socket_anywhere >/dev/null 2>&1; then
    socket=$(discover_wayland_socket_anywhere | head -n1 || true)
fi

if [ -n "${socket:-}" ] && command -v adopt_wayland_env_from_socket >/dev/null 2>&1; then
    log_info "Found Wayland socket: ${socket}"
    if ! adopt_wayland_env_from_socket "${socket}"; then
        log_fail "Failed to adopt Wayland env from ${socket}"
        printf 'FAIL %s\n' "${TESTNAME}" > "${RES_FILE}"
        exit 1
    fi
fi
