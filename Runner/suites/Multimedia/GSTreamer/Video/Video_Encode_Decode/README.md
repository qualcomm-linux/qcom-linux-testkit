...
### 2) Run ALL tests (basic + advanced) - LONG RUNNING

```bash
./run.sh --test-type all
```

**⚠️ WARNING:** `--test-type all` runs ALL 9+ tests and can take **10-15 minutes** with default settings:
- Basic encode/decode tests (H.264, H.265, VP9) - ~6 tests
- UVC camera preview (if camera available) - 1 test
- DRC H.264 decode (if clip available) - 1 test
- Concurrency tests: 8x H.264, 8x H.265, 2x MJPEG (if clips available) - 3 tests
- Advanced encode tests (if downstream stack + camera available) - 4 tests

**Time Budget (default 30s duration, h264,h265,vp9 codecs, 480p):**
- Basic tests: ~150s pipeline time + 30s setup = ~180s (3 minutes)
- Advanced tests: ~270s pipeline time + 60s setup/clips = ~330s (5.5 minutes)
- **Total: ~510s + overhead = 600-900 seconds (10-15 minutes)**

Tests that require missing hardware or clips will be gracefully skipped.

**For CI/LAVA:** See "CI/LAVA Time Budget Planning" section below for optimization strategies.
...
### 18) CI-friendly: Quick validation with reduced duration

```bash
./run.sh --test-type basic --duration 10 --codecs h264
```

### 19) CI-friendly: Run specific advanced test category

```bash
./run.sh --test-type concurrency --duration 15
```

---

## CI/LAVA Time Budget Planning
...
### Time Budget by Test Type (30s duration)

#### Basic Tests (default: h264,h265,vp9 @ 480p)
- **Encode**: 2 tests (h264, h265) × 30s = 60s
- **Decode**: 3 tests (h264, h265, vp9) × 30s = 90s
- **Setup**: ~30s (clip download if needed)
...
- **Setup**: ~60s (clip download, Wayland setup)
- **Total**: ~330s (5.5 minutes)

#### All Tests Combined
- **Pipeline time**: ~510s
- **Setup overhead**: ~90s
- **Total**: 600-900 seconds (10-15 minutes)

### CI Optimization Strategies

#### 1. Use Specific Test Types (Recommended)
Run only the tests you need:
```bash
# Quick smoke test (~3 min)
./run.sh --test-type basic --duration 10 --codecs h264

# Concurrency validation (~2 min)
./run.sh --test-type concurrency --duration 15

# Camera encode tests (~3 min)
./run.sh --test-type advanced-encode --duration 15
```

#### 2. Reduce Duration
Cut execution time by 66%:
```bash
# 10-second duration instead of 30
./run.sh --test-type all --duration 10
# Estimated time: 3-5 minutes instead of 10-15 minutes
```

#### 3. Limit Codecs
Test single codec for faster validation:
```bash
# H.264 only (~2-3 min for basic tests)
./run.sh --test-type basic --codecs h264 --duration 10
```

#### 4. Parallel Execution
Run test categories in parallel CI jobs:
```yaml
# Job 1: Basic tests
- ./run.sh --test-type basic --duration 10

# Job 2: Concurrency tests
- ./run.sh --test-type concurrency --duration 15

# Job 3: Advanced encode tests
- ./run.sh --test-type advanced-encode --duration 15
```
...
```bash
# 480p only (default, fastest)
./run.sh --test-type basic --resolutions 480p --duration 10
```
...
```bash
./run.sh --test-type basic --duration 10 --codecs h264 --resolutions 480p
```

#### Standard Validation (5-7 minutes)
```bash
./run.sh --test-type basic --duration 15 --codecs h264,h265
```

#### Comprehensive Test (10-15 minutes)
```bash
./run.sh --test-type all --duration 30
```

### LAVA Job Time Allocation

When configuring LAVA jobs, allocate time as follows:

| Test Configuration | Estimated Time | Recommended LAVA Timeout |
|-------------------|----------------|--------------------------|
| Basic (10s duration, h264 only) | 2-3 min | 5 min |
| Basic (15s duration, h264+h265) | 5-7 min | 10 min |
| Basic (30s duration, all codecs) | 3-5 min | 8 min |
| Concurrency (15s duration) | 2-3 min | 5 min |
| Advanced-encode (15s duration) | 3-4 min | 7 min |
| All tests (10s duration) | 3-5 min | 8 min |
| All tests (30s duration) | 10-15 min | 20 min |

**Note:** Add extra time for:
- Clip downloads (first run): +1-2 minutes
- Wayland setup: +30 seconds
- System variability: +20% buffer
