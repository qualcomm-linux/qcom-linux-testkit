...
get_test_clip() {
  ...
  if extract_tar_from_url "$clipUrl" "$OUTDIR"; then
    # extract_tar_from_url extracts to ${LOG_DIR:-.}, so check there first
    # LOG_DIR is set to logs/ by this suite, so search: logs/, current dir, and $OUTDIR
    if [ -f "${LOG_DIR:-}/$clip_name" ]; then
      mv "${LOG_DIR:-}/$clip_name" "$dest_path"
      log_info "Successfully downloaded and moved $clip_name from LOG_DIR"
      return 0
    elif [ -f "$clip_name" ]; then
      mv "$clip_name" "$dest_path"
      log_info "Successfully downloaded and moved $clip_name from current directory"
      return 0
    elif [ -f "$OUTDIR/$clip_name" ]; then
      log_info "Clip $clip_name already in correct location"
      return 0
    else
      log_warn "Clip $clip_name not found in downloaded content (checked: ${LOG_DIR:-}, ., $OUTDIR)"
      return 1
    fi
  else
