...
VIDEO_STACK="${VIDEO_STACK:-auto}" # also accepts base/overlay/up/down
VIDEO_PLATFORM="${VIDEO_PLATFORM:-}" # lemans|monaco|kodiak|"" (auto)
VIDEO_FW_DS="${VIDEO_FW_DS:-}" # Kodiak DS firmware path
VIDEO_FW_BACKUP_DIR="${VIDEO_FW_BACKUP_DIR:-}" # override, default /opt (set in lib)
...
export VIDEO_FW_DS VIDEO_FW_BACKUP_DIR VIDEO_NO_REBOOT VIDEO_FORCE LOG_DIR
...
while IFS= read -r cfg; do
    ...
    # Strict clip existence check (no subshell side effects)
    missing_case=0
