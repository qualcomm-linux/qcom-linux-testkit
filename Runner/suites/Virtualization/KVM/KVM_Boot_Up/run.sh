...
if ! check_kernel_config "CONFIG_VIRTUALIZATION"; then
    log_skip "$TESTNAME SKIP - CONFIG_VIRTUALIZATION is not enabled"
    echo "$TESTNAME SKIP" >"$res_file"
    exit 0
fi

if ! check_kernel_config "CONFIG_KVM"; then
    log_skip "$TESTNAME SKIP - CONFIG_KVM is not enabled"
    echo "$TESTNAME SKIP" >"$res_file"
    exit 0
fi
...
if [ "$dev_rc" -eq 2 ]; then
    log_skip "$TESTNAME SKIP - /dev/kvm is not available"
    echo "$TESTNAME SKIP" >"$res_file"
    exit 0
fi
...
log_info "=== KVM Boot Dmesg Validation ==="
...
log_pass "$TESTNAME PASS - KVM boot/runtime baseline is valid"
