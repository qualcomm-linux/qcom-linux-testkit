...
audio_evidence_pa_streaming() {
  ...
  command -v pactl >/dev/null 2>&1 || command -v pacmd >/dev/null 2>&1 || {
    # Final fallback: try to infer from our log if present. "Playing" is
    # intentionally excluded from this pattern - it is aplay's own generic
    # banner text (e.g. "Playing WAVE ..."), printed on every ALSA-backend
    # run regardless of PulseAudio involvement, so matching it here would
    # report PulseAudio streaming on pure-ALSA builds with no PulseAudio
    # daemon present at all. The remaining patterns are specific to
    # PulseAudio-aware tooling (paplay/parecord/pactl) and do not have this
    # ambiguity.
    if [ -n "${AUDIO_LOGCTX:-}" ] && [ -s "$AUDIO_LOGCTX" ]; then
      grep -qiE 'Connected to PulseAudio|Opening audio stream|Stream started|Starting recording' "$AUDIO_LOGCTX" && { echo 1; return; }
    fi
    echo 0; return
  }
...
EOF
  ...
  # Last resort: infer from our player/recorder logs. "Playing" is excluded
  # here for the same reason noted in the quick-exit fallback above - it is
  # aplay's own generic banner text, not evidence of PulseAudio involvement.
  if [ -n "${AUDIO_LOGCTX:-}" ] && [ -s "$AUDIO_LOGCTX" ]; then
    grep -qiE 'Connected to PulseAudio|Opening audio stream|Stream started|Starting recording' "$AUDIO_LOGCTX" && { echo 1; return; }
  fi
...
audio_exec_with_timeout() {
  ...
  if kill -0 "$pid" 2>/dev/null; then
    ...
    # A killed process may exit with the raw signal code (143) or with
    # its own status if it caught SIGTERM and exited on an interrupted
    # syscall (e.g. aplay/arecord commonly report 1 here). Either way
    # the deadline was reached, so normalize to 124 for the caller.
    wait "$pid" 2>/dev/null
    return 124
  fi
