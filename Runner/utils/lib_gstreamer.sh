...
# Enforces SIGINT grace period (10s) followed by SIGKILL to prevent wedged V4L2 ioctls.
...
gstreamer_run_gstlaunch_timeout() {
  ...
  # REQUIRE timeout command - reject unbounded fallback
  ...
  if [ "$secs" -gt 0 ] 2>/dev/null; then
    # Send SIGINT to trigger EOS via -e flag, with 10s grace period before SIGKILL
    # This prevents wedged V4L2 ioctls from consuming entire LAVA action timeout
    # shellcheck disable=SC2086
    timeout --signal=INT --kill-after=10 "$secs" "$GSTBIN" $GSTLAUNCHFLAGS $pipe
    return $?
  fi

  # If secs <= 0, run without timeout (but still require timeout command to exist)
  # shellcheck disable=SC2086
