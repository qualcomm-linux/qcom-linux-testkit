...
# extract_tar_from_url, scan_dmesg_errors, (optional) run_with_timeout.
...
video_normalize_stack() {
    # outputs: auto|upstream|downstream (maps base/overlay/up/down)
...
video_auto_preference_from_blacklist() {
    plat="$1"
    # If persistent policy blacklists upstream bits, prefer downstream.
    case "$plat" in
        lemans|monaco)
...
video_remove_blacklist() {
	...
	tok_re=$(printf '%s' "$tok" | sed 's/[][(){}.^$|*+?\\/]/\\&/g')
        sed "/^[[:space:]]*blacklist[[:space:]]\+${tok_re}[[:space:]]*$/d" "$BLACKLIST_FILE" >"$tmp" 2>/dev/null \
...
# -----------------------------------------------------------------------------
video_log_fw_hint() {
    # Best-effort: surface firmware-related lines so users see the blob picked up.
    ...
    case "$plat" in
        ...
        kodiak)
            if [ "$stack" = "downstream" ]; then
                ...
                # Load downstream → request_firmware() new blob
...
video_ensure_stack() {
    ...
    if [ "$want" = "auto" ]; then
        # 1) persistent policy via blacklist
...
video_ensure_clips_present_or_fetch() {
    ...
    if [ -s "$tmp_list" ]; then
        log_warn "Some input clips are missing (see $tmp_list)"
        [ -z "$tu" ] && tu="$TAR_URL"
        if [ -n "$tu" ]; then
            log_info "Attempting fetch via TAR_URL=$tu"
            extract_tar_from_url "$tu" || log_warn "Fetch/extract failed for TAR_URL"
        else
...
video_run_once() {
    ...
    if video_have_run_with_timeout; then
        if run_with_timeout "$tmo" iris_v4l2_test --config "$cfg" --loglevel "$lvl" >>"$logf" 2>&1; then :; else
            rc=$?
            if [ "$rc" -eq 124 ] 2>/dev/null; then log_fail "[run] timeout after ${tmo}s"; else log_fail "[run] iris_v4l2_test exited rc=$rc"; fi
            return 1
        fi
    else
        if iris_v4l2_test --config "$cfg" --loglevel "$lvl" >>"$logf" 2>&1; then :; else
            rc=$?
